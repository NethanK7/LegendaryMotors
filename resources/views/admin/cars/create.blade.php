    ...
    <div class="pt-32 pb-12 bg-black min-h-screen text-white">
        <div class="container mx-auto px-6 max-w-4xl">
            ...
            <div class="flex items-center justify-between mb-8">
                <h1 class="text-3xl font-black uppercase tracking-tighter">Add New Vehicle</h1>
                ...
                <form action="{{ route('admin.cars.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="brand" value="{{ old('brand') }}" class="w-full bg-black border border-white/10 px-4 py-3 text-white focus:border-[var(--color-accent)] focus:ring-0 uppercase font-bold" required>
                        </div>
                        
                        <!-- Model -->
                        <div>
                            <label class="block text-gray-500 text-xs font-bold uppercase tracking-widest mb-2">Model</label>
                            <input type="text" name="model" value="{{ old('model') }}" class="w-full bg-black border border-white/10 px-4 py-3 text-white focus:border-[var(--color-accent)] focus:ring-0 uppercase font-bold" required>
                        </div>

                        <!-- Year -->
                        <div>
                            <label class="block text-gray-500 text-xs font-bold uppercase tracking-widest mb-2">Year</label>
                            <input type="number" name="year" value="{{ old('year') }}" class="w-full bg-black border border-white/10 px-4 py-3 text-white focus:border-[var(--color-accent)] focus:ring-0 font-tech" required>
                        </div>

                        <!-- Price -->
                        <div>
                            <label class="block text-gray-500 text-xs font-bold uppercase tracking-widest mb-2">Price ($)</label>
                            <input type="number" step="0.01" name="price" value="{{ old('price') }}" class="w-full bg-black border border-white/10 px-4 py-3 text-white focus:border-[var(--color-accent)] focus:ring-0 font-tech" required>
                        </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="image_url" value="{{ old('image_url') }}" class="w-full bg-black border border-white/10 px-4 py-3 text-white focus:border-[var(--color-accent)] focus:ring-0 text-sm" placeholder="images/brabus/..." required>
                    </div>
                    ...
                    <div class="border-t border-white/10 pt-6 mt-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <input type="number" name="hp" value="{{ old('hp') }}" class="w-full bg-black border border-white/10 px-4 py-3 text-white focus:border-[var(--color-accent)] focus:ring-0 font-tech">
                            </div>

                            <!-- 0-60 -->
                            <div>
                                <label class="block text-gray-500 text-xs font-bold uppercase tracking-widest mb-2">0-60 MPH (s)</label>
                                <input type="text" name="0_60" value="{{ old('0_60') }}" class="w-full bg-black border border-white/10 px-4 py-3 text-white focus:border-[var(--color-accent)] focus:ring-0 font-tech">
                            </div>

                            <!-- Top Speed -->
                            <div>
                                <label class="block text-gray-500 text-xs font-bold uppercase tracking-widest mb-2">Top Speed (MPH)</label>
                                <input type="text" name="top_speed" value="{{ old('top_speed') }}" class="w-full bg-black border border-white/10 px-4 py-3 text-white focus:border-[var(--color-accent)] focus:ring-0 font-tech">
                            </div>
                        </div>
                    </div>
